...
after <- ls()
model_name <- setdiff(after, before)

if ("HD" %in% colnames(midterm_predictions)){
  if (model_name[2]=="globalmodel"){
    future_predictions <- stats::predict(globalmodel, newx = as.matrix(new_data[,9:43]))}
  else{
...
mt_plot <- ggplot(all_data)+geom_line(aes(1:nrow(all_data),all_data$seasonal_avg_hourly_demand,color="actual"))+
  geom_line(aes(1:nrow(all_data),midterm_model_fit,color="fitted"))+
...
suppressWarnings(
ggsave(file=paste0("./",unique(all_data$country),"/plots/mid_term_results_future.png"), plot=mt_plot, width=12, height=8)
)
suppressWarnings(
print(mt_plot)
)
